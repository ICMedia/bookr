...
export default class MyBookingsPage extends React.Component {
			...
			<div>
				<div className="jumbotron">
					<div className="container">
						<h1>{this.renderName()} Bookings</h1>
					</div>
				</div>
	...
	onUserChange() {
		...
		if (userInfo.loggedIn) {
			BookingActions.listAll({created_by: userInfo.username});
		}
	}

	onBookingsChange() {
		this.setState({
			bookings: BookingStore.listForQuery({created_by: this.state.user.username}),
			loading: false
		});
	}
}
